...
* `on_failed_health_check` - (Optional [Beta](https://terraform.io/docs/providers/google/guides/provider_versions.html)), Default behavior for all instance in case of health check failures. Valid options are: `DEFAULT_ACTION`, `REPAIR`, `DO_NOTHING`. If `DO_NOTHING` then instances will not be repaired. If `REPAIR` then unhealthy instances will be repaired. If `DEFAULT_ACTION` (default), then action from `default_action_on_failure` will be applied.
